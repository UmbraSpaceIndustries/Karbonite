...
{
    public class ORSHeatMap : MonoBehaviour
    {
        static Dictionary<string, ORSHeatMapInfo> body_resource_maps = new Dictionary<string, ORSHeatMapInfo>();
        static Dictionary<string, Vector2d[]> body_abudnance_angles = new Dictionary<string, Vector2d[]>();
        static List<ORSHeatMapOverlay> abundance_markers = new List<ORSHeatMapOverlay>();
        ...
        static Vector3d sphere_scale = new Vector3d(205000, 205000, 205000);  //Little Map, bigger is better
        static Vector3d sphere_scale_scaled = new Vector3d(205, 205, 205);  //Big Map, lower is smaller - 2 = default
        ...
        public static void loadPlanetaryResourceData(int body)
        {
            ...
            foreach (UrlDir.UrlConfig config in configs)
            {
                ...
                if (planetary_resource_config_node.GetValue("celestialBodyName") == celestial_body_name && planetary_resource_config_node != null)
                {
                    ...
                    ORSHeatMapInfo resource_info = new ORSHeatMapInfo(resource_gui_name, map, body);
                    ...
                    //    resource_info.setDisplayTexture(tex_path);
                    //}
                    string tex_path = planetary_resource_config_node.GetValue("mapUrl");
                    resource_info.setDisplayTexture(tex_path);
                    ...
                    //}
                    //TEMP
                    abundance_points_list.Add(new Vector2d(90,0));
        ...
        //protected static double getPixelAbundanceValue(int pix_x, int pix_y, ORSHeatMapInfo resource_info)
        //{
        ...
        //    if (resource_info.getResourceScale() == ORSHeatMapInfo.LOG_SCALE)
        //    {
        //        resource_val = Math.Pow(scale_factor, pix_color.grayscale * 255.0) / 1000000 * scale_multiplier;
        //    }
        //    else if (resource_info.getResourceScale() == ORSHeatMapInfo.LINEAR_SCALE)
        //    {
        ...
        //    try
        //    {
        //        ORSHeatMapInfo resource_info = body_resource_maps.Where(ri => ri.Value.getResourceName() == resourcename).FirstOrDefault().Value;
        ...
        //    {
        //        ORSHeatMapInfo resource_info = body_resource_maps[resourcename];
        //        Texture2D map = resource_info.getResourceMap();
        ...
        {
            ORSHeatMap.displayed_resource = displayed_resource;
            foreach (ORSHeatMapOverlay abundance_marker in abundance_markers)
            {
                removeAbundanceSphere(abundance_marker.getPlanetarySphere());
                removeAbundanceSphere(abundance_marker.getScaledSphere());
        ...
        protected static GameObject createAbundanceSphere()
        {
            if (sphere == null)
            {
                ...
                //resource_prim.transform.localScale = sphere_scale * (FlightGlobals.currentMainBody.Radius / FlightGlobals.Bodies[ORSGameConstants.REF_BODY_KERBIN].Radius);
                resource_prim.transform.localScale = sphere_scale_scaled * (FlightGlobals.currentMainBody.Radius / FlightGlobals.Bodies[ORSGameConstants.REF_BODY_KERBIN].Radius);
                resource_prim.renderer.material.shader = Shader.Find("Colormap");
                var mTex = GameDatabase.Instance.GetTexture(sphere_texture, false);
                var cTex = GameDatabase.Instance.GetTexture(@"UmbraSpaceIndustries/Karbonite/ORS/gradient", false);
                resource_prim.renderer.material.SetTexture("_MainTex", cTex);
                resource_prim.renderer.material.SetTexture("_ColorTex", mTex);
                resource_prim.renderer.material.color = new Color(Color.red.r, Color.green.g,Color.blue.b, 1f);
